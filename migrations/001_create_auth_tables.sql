-- =====================================================================
-- MIGRATION STACGATELMS : SYSTÈME D'AUTHENTIFICATION (VERSION CORRIGÉE)
-- =====================================================================
...
-- =====================================================================
-- TABLE RÔLES (À CRÉER EN PREMIER pour les références)
-- =====================================================================
...
CREATE TABLE sg_roles (
    ...
    name VARCHAR(50) NOT NULL UNIQUE COMMENT 'Nom du rôle',
    display_name VARCHAR(100) NOT NULL COMMENT 'Nom affiché',
    description TEXT NULL COMMENT 'Description du rôle',
    level TINYINT UNSIGNED NOT NULL DEFAULT 1 COMMENT 'Niveau hiérarchique',
    color VARCHAR(7) NOT NULL DEFAULT '#6c757d' COMMENT 'Couleur d\'affichage',
    icon VARCHAR(50) NULL COMMENT 'Icône FontAwesome',
    is_active TINYINT(1) NOT NULL DEFAULT 1 COMMENT '1=actif, 0=désactivé',
    is_system TINYINT(1) NOT NULL DEFAULT 0 COMMENT '1=rôle système',
  ...
  COMMENT='Rôles système avec hiérarchie';
...
CREATE TABLE sg_users (
    ...
    email VARCHAR(100) NOT NULL UNIQUE COMMENT 'Email unique pour connexion',
    password_hash VARCHAR(255) NOT NULL COMMENT 'Hash sécurisé du mot de passe',
    first_name VARCHAR(50) NULL COMMENT 'Prénom utilisateur',
    last_name VARCHAR(50) NULL COMMENT 'Nom de famille utilisateur',
    avatar_url VARCHAR(255) NULL COMMENT 'URL de l\'avatar utilisateur',
    status TINYINT(1) NOT NULL DEFAULT 1 COMMENT '1=actif, 0=désactivé, -1=banni',
    email_verified TINYINT(1) NOT NULL DEFAULT 0 COMMENT '1=email vérifié, 0=en attente',
    login_attempts TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Nombre de tentatives échouées',
    last_login_at TIMESTAMP NULL COMMENT 'Dernière connexion réussie',
    last_login_ip VARCHAR(45) NULL COMMENT 'IP de dernière connexion',
    password_reset_token VARCHAR(64) NULL COMMENT 'Token de réinitialisation',
    password_reset_expires TIMESTAMP NULL COMMENT 'Expiration du token',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (id),
  ...
  COMMENT='Comptes utilisateurs StacGateLMS';
...
CREATE TABLE sg_permissions (
    id INT UNSIGNED NOT NULL AUTO_INCREMENT,
    name VARCHAR(100) NOT NULL UNIQUE COMMENT 'Nom technique de la permission',
    display_name VARCHAR(150) NOT NULL COMMENT 'Nom affiché dans l\'interface',
    description TEXT NULL COMMENT 'Description de la permission',
    module VARCHAR(50) NOT NULL COMMENT 'Module concerné',
    resource VARCHAR(50) NOT NULL COMMENT 'Ressource cible',
    action VARCHAR(30) NOT NULL COMMENT 'Action autorisée',
    category VARCHAR(50) NULL COMMENT 'Catégorie pour regroupement',
    sort_order INT UNSIGNED NOT NULL DEFAULT 999 COMMENT 'Ordre d\'affichage',
    is_active TINYINT(1) NOT NULL DEFAULT 1 COMMENT '1=permission active',
    is_system TINYINT(1) NOT NULL DEFAULT 0 COMMENT '1=permission système',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (id),
  ...
  COMMENT='Permissions granulaires pour contrôle d\'accès';
...
CREATE TABLE sg_user_roles (
  ...
  COMMENT='Association utilisateurs-rôles';
...
CREATE TABLE sg_role_permissions (
  ...
  COMMENT='Association rôles-permissions';
...
-- Insertion des permissions essentielles
